    ...
    return (
        <div className="p-6 bg-white shadow-lg rounded-lg h-screen">
            ...
            <div className={`flex mb-5 border-b border-gray-300 `}>
                {(Object.keys(tabLabels) as Tab[]).map((t) => (
                    <button
                        ...
                        className={`px-5 py-2 font-medium border-b-2 transition-colors duration-200 ${user.permissions?.includes(t) ? "" : "hidden"} ${tab === t
            ...
            {(Object.keys(tabLabels) as Tab[]).map((t) => (
                <div
                    key={t}
                    className={`${user.permissions?.includes(tabLabels[t]) ? "" : "hidden"}`}
                >
                    {/* Search */}
